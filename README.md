...
# Fedora 44
curl -O https://cdn.posit.co/r/fedora-44/pkgs/R-${R_VERSION}-1-1.$(arch).rpm
```
...
package per platform and architecture, old nightlies are cleaned up by a
Cloudsmith retention policy on the repository that matches only the daily
builds (`R-next`/`R-devel` and `r-next`/`r-devel`) and keeps them for one day.
Stable release packages are not covered by the policy and are never removed.
