...
IDEs discover R installations by parsing `bin/R` as a text file rather than executing it.
Two things that can break IDE compatibility:

- **`R_HOME_DIR` must be parseable from `bin/R`**: Positron's R discovery
  (`getRHomePathLinux()`) reads `bin/R` line-by-line looking for a static `R_HOME_DIR=<path>`
  assignment and the `if test "${R_HOME_DIR}"` block. The relocatable override must be
  appended *after* this block (not replace it), so parsers see the standard R script
  structure while the dynamic override takes effect at runtime.

- **`CURL_CA_BUNDLE` and `TCL_LIBRARY`/`TK_LIBRARY` must be set outside `etc/ldpaths`**:
  RStudio's rsession loads `libR.so` directly and does not source `etc/ldpaths` as a shell
  script. Any environment variables that need to be available in R sessions launched by IDEs
  must be set in the base Rprofile (`library/base/R/Rprofile`) or `Rprofile.site`, not in
  `ldpaths`. The base Rprofile is preferred because it also works with `--vanilla` (which
  disables `Rprofile.site`).
